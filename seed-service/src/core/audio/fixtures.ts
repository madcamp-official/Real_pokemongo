...
/**
 * 백색잡음. 구형 FFmpeg에도 있는 aevalsrc의 random()을 사용한다. 최신 FFmpeg 전용
 * anoisesrc를 쓰면 2013년판이 설치된 Windows 개발 환경에서 테스트 픽스처 생성 자체가
 * 실패하므로, 같은 신호를 더 넓은 버전에서 만들 수 있는 식으로 표현한다.
 */
export function makeWhiteNoise(seconds: number, amplitude: number): Promise<Buffer> {
  return runFfmpegToFile([
    "-y",
    "-f", "lavfi",
    "-i", `aevalsrc=${amplitude}*(2*random(0)-1):s=48000:d=${seconds}`,
    "-t", String(seconds),
    ...WAV_ARGS,
  ]);
}
