...
**목적**: 아이가 직접 밖에 나가 생물을 관찰·촬영·녹음하면 AI가 종을 동정해 도감에 기록해주고, 확률형(가챠) 보상 없이 "실제 관찰"만으로 퀘스트·배지·경험치를 얻게 해서 자연 관찰 자체를 동기부여로 삼는다. 아동 대상 서비스이므로 프라이버시(정밀 위치·사진 메타데이터 처리)와 안전(위험 생물 경고, 식용 여부 판단 금지)을 설계 원칙 수준에서 강제한다.

**핵심 기능**: 사진 촬영 → BioCLIP 기반 이미지 동정, 소리 녹음 → BirdNET 기반 조류 소리 동정, 동정된 종은 실제 GPS 좌표와 함께 지도 핀으로 기록되고 도감(Dex)에 해금된다. 해금한 종마다 "개체(친구)"가 생겨 홈가든에 배치하고 상호작용하며 유대감을 쌓을 수 있고, 관찰 기반 퀘스트·배지로 보상을 받는다. "도감 박사" 챗봇에게 종에 대해 질문하면 검수된 지식 문장을 의미 기반 검색으로 찾아 답해준다.

**예상 사용자**: 초등학생 등 아동, 그리고 아이와 함께 야외 활동을 하는 보호자.

**차별점**:
- 사진뿐 아니라 **소리(조류 울음)까지 동정 모달리티를 확장**했고, 두 파이프라인 모두 확신도 등급(high/medium/low/unknown)과 안전장치(노이즈·품질 게이트, 혼동종 근접 시 사용자 확인 유도)를 갖췄다.
- "도감 박사" 챗봇은 생성형 LLM이 아니라 **검수된 문장에 대한 의미 기반 검색 + 안전 답변 하드코딩** 구조라, 아동에게 검증되지 않은 내용을 지어내지 않는다.
- 프라이버시가 문서 수준이 아니라 **타입 시스템 수준으로 강제**된다 — 정화(EXIF 제거)되지 않은 원본 사진은 애초에 컴파일되는 타입 자체가 없고, 위치는 "동의 시에만 저장되는 일반화된 지역"과 "동의와 무관하게 항상 저장되는 정밀 좌표(지도 핀용)"를 별개 필드로 분리했다.
...
- [ ] 팀/워크스페이스 협업, 실시간 동기화(WebSocket) — 단일 사용자 계정 구조라 애초에 요구사항이 아님(코드 전수 조사 결과 WebSocket 관련 의존성·라우트 전무 확인)
- [ ] 생성형 LLM 기반 자유 대화 — "도감 박사"는 사전 구축된 문장 인덱스에 대한 검색이지 LLM 호출이 아님
...
이 앱은 **탭바 UI 자체를 렌더링하지 않는다** — 지도 화면이 홈이고, 다른 화면들은 지도 하단의 방사형 메뉴(RadialMenu, 포켓몬고 스타일)로 진입한다. React Navigation의 바텀탭은 화면 상태 보존과 하드웨어 뒤로가기 처리 목적으로만 내부적으로 유지된다.
...
Base path 없음(라우트 그대로 사용), 인증은 `Authorization: Bearer <JWT>`.
...
**실시간 통신(WebSocket) 없음** — 전수 조사 결과 관련 의존성·라우트가 전혀 없습니다. 단일 사용자 계정 구조라 실시간 협업/브로드캐스트 요구사항 자체가 없기 때문입니다.
...
**서비스 URL**: (없음 — 실제 공개 배포가 확인되지 않았습니다)

현재 코드 조사 결과, Dockerfile/docker-compose/CI-CD 파이프라인/리버스 프록시/도메인 설정이 전혀 없습니다. DB(PostgreSQL)·BioCLIP·소리 동정 모델은 전부 사설 GPU 서버(사설 IP, `127.0.0.1`에만 바인딩)에 떠 있고, 개발자가 SSH 로컬 포트포워딩 터널을 직접 열어야만 접근할 수 있습니다.
